              ...
              try {
                ...
                if (operation === 'findMany') {
                  if (prop === 'activity') return [
                    { id: "mock-act-1", name: "Mock Survey", category: "Survey", date: "2024-01-01", startTime: "10:00", endTime: "12:00", location: "City Center", volunteersCount: 5, capacity: 10, status: "In Progress", area: { name: "Muzaffarnagar" }, teamLeader: { user: { name: "Amit Kumar" } } }
                  ];
                  if (prop === 'issue') return [
                    { id: "mock-iss-1", category: "Water", priority: "High", description: "Water shortage reported", dateReported: "2024-01-01", status: "Open", area: { name: "Muzaffarnagar" }, reportedBy: { name: "Volunteer 1" }, assignedTo: { name: "Suresh" } }
                  ];
                  if (prop === 'groundReport') return [
                    { id: "mock-gr-1", date: "2024-01-01", location: "Sector 4", participantCount: 50, status: "Submitted", activity: { name: "Survey" }, volunteer: { user: { name: "Vikram" } } }
                  ];
                  ...
                  if (prop === 'ward') return [
                    { id: "mock-ward-1", name: "Ward 1", type: "Ward", population: 5000, households: 1000, area: { name: "Muzaffarnagar" } }
                  ];
                  if (prop === 'pollingStation') return [
                    { id: "mock-ps-1", name: "PS 1", number: 1, address: "Main School", recordCount: 500, status: "Validated", area: { name: "Muzaffarnagar" }, teamLeader: { user: { name: "Amit" } } }
                  ];
...
// Only wrap with mock proxy in development/test when DISABLE_MOCKS is not true
const useMocks = process.env.NODE_ENV !== "production" && process.env.DISABLE_MOCKS !== "true";

export const prisma = useMocks
