...
import { Shield, Map, Users, Activity, AlertTriangle, Eye, Radio, Zap, Clock, CheckCircle2, UserCheck } from "lucide-react";
import { mockDashboardStats, mockAreas, mockActivities, mockIssues } from "@/lib/mock-data";

export default function WarRoomPage() {
  const activeActivities = mockActivities.filter(a => a.status === "In Progress");
  const overdueActivities = mockActivities.filter(a => a.status === "Overdue");
  const criticalIssues = mockIssues.filter(i => i.priority === "Critical" || i.priority === "High");
      ...
      <div className="grid gap-3 grid-cols-2 sm:grid-cols-4">
        <Card className="border-l-4 border-l-emerald-500">
          <CardContent className="p-4">
            ...
            <p className="text-2xl font-bold">{mockDashboardStats.activeUsers}</p>
            <p className="text-xs text-muted-foreground mt-0.5">of {mockDashboardStats.volunteers + mockDashboardStats.teamLeaders} total</p>
          </CardContent>
        </Card>
        <Card className="border-l-4 border-l-blue-500">
          <CardContent className="p-4">
            ...
            <p className="text-xs text-muted-foreground mt-0.5">{mockActivities.filter(a => a.status === "Scheduled").length} upcoming</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            ...
            <div className="p-3 rounded-lg bg-orange-50 dark:bg-orange-900/10 border border-orange-100 dark:border-orange-900/20">
              ...
              <p className="text-xs text-orange-600/80 dark:text-orange-400/70">14 volunteers inactive for 48+ hours</p>
            </div>
            <div className="p-3 rounded-lg bg-blue-50 dark:bg-blue-900/10 border border-blue-100 dark:border-blue-900/20">
              ...
              <p className="text-xs text-blue-600/80 dark:text-blue-400/70">3 reports awaiting verification</p>
            </div>
          </CardContent>
        </Card>
      </div>
            ...
            {mockAreas.filter(a => a.status === "Active").slice(0, 4).map((area) => (
              <div key={area.id} className="p-3 rounded-lg border space-y-2">
                ...
                <div className="grid grid-cols-3 gap-1 text-center text-xs">
                  <div><p className="font-semibold">{area.teamLeaders}</p><p className="text-muted-foreground">TLs</p></div>
                  <div><p className="font-semibold">{area.volunteers}</p><p className="text-muted-foreground">Vols</p></div>
                  <div><p className="font-semibold">{area.pollingStations}</p><p className="text-muted-foreground">PS</p></div>
                </div>
              </div>
            ))}
          </div>
