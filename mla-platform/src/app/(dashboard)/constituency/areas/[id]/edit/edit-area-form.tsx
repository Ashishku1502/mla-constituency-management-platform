...
export function EditAreaForm({ area, managers }: EditAreaFormProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    managerId: area.managerId || "unassigned"
  });

  const [geographicBoundary, setGeographicBoundary] = useState(area.geographicBoundary || "");
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok && result.success) {
        ...
        router.push(`/constituency/areas/${area.id}`);
        router.refresh();
      } else {
  ...
  return (
    <div className="space-y-6 animate-in fade-in-50 duration-500">
      <PageHeader
        title="Edit Area"
        description={`Update details and boundaries for ${area.name}`}
        icon={Building2}
      />
            ...
            <CardHeader className="pb-3 border-b shrink-0 flex flex-row items-center justify-between">
              <div>
                ...
                <CardDescription className="mt-1">
                  Use the map tools to draw or update the boundaries for this Area.
                </CardDescription>
              </div>
